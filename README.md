...
#### Generating slugs from IDs
When slugs are not custom they are generated by converting the integer ID of
the link record into a high base number, using alphanumeric characters that I
find easy to read uniquely even when handwritten. (See the
`NON_CONFUSING_CHARACTERS` constant in `link.rb`).

I considered a few way to generate slugs from URLs. This approach had a couple
of advantages:
1. It will necessarily be the shortest length available with the character set
2. Before the introduction of custom slugs, there is no possibility of
   collision (unlike if we used a hash function).

I handled the possibility of collision between a custom slug and one of these
automatically generated slugs in what I admit is a rather quirky way: if a
slug would be a collision, it uses the ID of the link whose slug it collided
with -- and then recursively continuing that, for the extremely unlikely case
